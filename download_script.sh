...
if [ -f ${entree} ]; then
        command="/usr/bin/plowdown -r 10 -x -m --9kweu=I1QOR00P692PN4Q4669U --temp-rename --temp-directory ${destination_temp} -o ${destination} ${e$
        echo ${command}
		...
		if [ $? -eq 1  ]; then
                        echo "**** Nom du screen : ${nomScreen}"
                        # command="/usr/bin/plowdown -r10 -x -m --9kweu=I1QOR00P692PN4Q4669U --temp-rename --temp-directory ${destination_temp} -o $$
